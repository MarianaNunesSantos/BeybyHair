...
$subcategoria = isset($_GET['subcategoria']) ? urldecode($_GET['subcategoria']) : '';

$conexao = new Conexao();
$pdo = $conexao->fazConexao();

try {
    // Busca os produtos da subcategoria selecionada
    $stmt = $pdo->prepare("
        SELECT * FROM produto 
        WHERE subcategoriaPro = ?
        ORDER BY nomePro
    ");
    $stmt->execute([$subcategoria]);
    $produtos = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Verifica se a subcategoria existe (mesmo sem produtos)
    $stmt = $pdo->prepare("SELECT DISTINCT subcategoriaPro FROM produto WHERE subcategoriaPro = ?");
    $stmt->execute([$subcategoria]);
    $subcategoriaExiste = ($stmt->rowCount() > 0);
...
<head>
    ...
    <!-- Link para o CSS do Bootstrap -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <title><?= htmlspecialchars($subcategoria) ?> - BeybyHair</title>
</head>
...
<body>
    ...
    <main class="col-md-12 container-fluid d-flex justify-content-center">
        ...
        <section class="col-md-8 col-12">
            <h1 class="mb-4 titulo"><?= htmlspecialchars($subcategoria) ?></h1>
            <hr>
